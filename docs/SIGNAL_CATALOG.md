...
| `AppFocusHeartbeat` | `ApplicationUsageCollector` | StateSample | every 15s while a dwell is open | `appKey`, `category`, `confidence`, `dwellStartUtc` | Emitted periodically during sustained foreground focus. Enables `AppFeatureAggregator` to synthesize open-dwell overlap segments for windows where `AppDwell` has not yet fired. `dwellStartUtc` is the ISO-8601 round-trip (`"O"`) start of the current foreground slice. |
